    ...
    <div className="min-h-screen bg-surface-low">
      ...
      <nav className="bg-white border-b border-ink-100 px-6 lg:px-10 py-4 flex items-center justify-between sticky top-0 z-10 shadow-sm">
        ...
        <div className="flex items-center gap-3">
          ...
          >
            <span className="material-symbols-outlined text-brand-500 text-xl">person</span>
          </button>
        </div>
      </nav>
      ...
      <main className="max-w-5xl mx-auto px-6 lg:px-10 py-8">
        {/* Welcome */}
        <div className="mb-6">
          <h1 className="font-display text-2xl font-black text-ink-800 mb-1">Halo! 👋</h1>
        ...
        {formData.childName && (
          <div className="card mb-6 flex items-center gap-4">
            <div className="w-12 h-12 bg-brand-100 rounded-2xl flex items-center justify-center shrink-0">
              <span className="material-symbols-outlined text-brand-500 text-2xl" style={{ fontVariationSettings: "'FILL' 1" }}>
                child_care
              </span>
            </div>
            <div>
              <p className="font-display font-bold text-ink-800">{formData.childName}</p>
              <p className="text-xs text-ink-400">Lahir {birthMonthName} {birthYear}</p>
              <p className="text-xs text-brand-500 font-medium mt-0.5">Usia: {ageDisplay}</p>
            </div>
          </div>
        )}
        ...
        <div className="card text-center py-16 lg:py-20">
          <div className="w-24 h-24 bg-brand-50 rounded-full flex items-center justify-center mx-auto mb-5 border-2 border-brand-100">
            <span className="material-symbols-outlined text-brand-500 text-4xl" style={{ fontVariationSettings: "'FILL' 1" }}>
              calculate
            </span>
          </div>
          <h2 className="font-display text-xl font-black text-ink-800 mb-2">Fitur Segera Hadir</h2>
